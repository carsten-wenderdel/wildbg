...
fn rollout_close_to_race(c: &mut Criterion) {
    ...
    let rollout =
        RolloutEvaluator::with_evaluator_and_seed(OnnxEvaluator::contact_default_tests(), 123456);
...
fn rollout_early_game(c: &mut Criterion) {
    ...
    group.sample_size(10);
    let rollout =
        RolloutEvaluator::with_evaluator_and_seed(OnnxEvaluator::contact_default_tests(), 12121212);
